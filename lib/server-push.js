...
// Generated once per PROCESS start. The server watchdog counts distinct ids
// to detect restart loops — a crash-looping agent cannot report its own
// death, so the restart signal must be inferable server-side from pushes.
...
async function pushMetricsToServer() {
  ...
  try {
    ...
    // Collect all metrics in parallel. Alerts are no longer collected or
    // pushed (2026-08-12) — no live cluster runs vmalert, so this snapshot
    // type was always empty; the server's alert-ingest path was retired
    // alongside it.
    const [overview, node, pod, pvc, ns] = await Promise.all([
