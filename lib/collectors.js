...
async function getPodData() {
  ...
  (ownerData || []).forEach((o) => {
    // Same namespace fallback the pod key uses below. kube_pod_owner comes from
    // kube-state-metrics, whose labels land under exported_namespace on some
    // setups; keying on metric.namespace alone silently produced
    // "undefined/<pod>" there, so EVERY pod lost its owner and fell back to
    // bare. Both sides of the lookup must resolve the namespace identically.
    const key = `${o.metric.exported_namespace || o.metric.namespace}/${o.metric.pod}`;
    ownerMap.set(key, o);
  });
  ...
  for (const rs of rsOwnerData || []) {
    const ns = rs.metric.exported_namespace || rs.metric.namespace;
