...
async function ensureMonitoringStack() {
  ...
  // Self-heal the agent's OWN ClusterRole so an image update can add newly
  // required permissions (e.g. `events`, added 2026-07-16 so the Doctor can
  // read cluster events) WITHOUT the operator re-applying the install YAML.
  // Best-effort: on managed clusters (GKE/EKS/AKS) an SA can't escalate its
  // own role for a permission it lacks yet — that fails here, is caught, and
  // the operator re-runs the one-command install (applied with their creds).
  try {
    const selfRbac = path.join(
      __dirname,
      "..",
      "..",
      "manifests",
      "moniple-agent-rbac.yaml",
    );
    if (fs.existsSync(selfRbac)) {
      await ensureRbacFromManifest(selfRbac, namespace);
      console.log("Agent RBAC reconciled (self-heal)");
    }
  } catch (e) {
    console.warn(
      `Agent RBAC self-heal skipped: ${e.message} ` +
        "(re-apply the install YAML if the Doctor can't read events)",
    );
  }
